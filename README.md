...
`test/marketentry/facts_test.cljk`'s
...
```bash
kbb -M:dev:run     # walk a clean intake -> assess -> draft -> submit lifecycle, plus HARD-hold scenarios (PRODHAB registration-scope mismatch included)
...
- `src/marketentry/{facts,governor,phase,sim,operation,registry,store,
  ...
  own `src/statute/facts.cljk` citations verbatim (Código de Comercio
  ...
  `test/marketentry/governor_contract_test.cljk`'s three contrasting
