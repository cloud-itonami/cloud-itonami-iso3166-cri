  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Consultores San José S.R.L."}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Consultores San José S.R.L." (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest prodhab-registration-scope-mismatch-is-held-and-unoverridable
  (testing "commercialize scope, not PRODHAB-registered -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:prodhab-registration-scope-mismatch} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))

(deftest prodhab-registration-scope-mismatch-does-not-fire-when-registered
  (testing "SCOPE-CONDITIONAL: the SAME commercialize scope, but registered with PRODHAB, must NOT trigger the flagship check"
    (let [[db actor] (fresh)
          _ (assess! actor "t5bpre" "eng-5")
          _ (draft! actor "t5bpre" "eng-5")]
      (is (= :commercialize (:data-handling-scope (store/engagement db "eng-5"))) "fixture really declares commercialize scope")
      (is (true? (:prodhab-registered? (store/engagement db "eng-5"))) "fixture really is PRODHAB-registered")
      (let [r1 (exec-op actor "t5b" {:op :filing/submit :subject "eng-5"} operator)]
        ...
        (let [r2 (approve! actor "t5b")]
          (is (= :commit (get-in r2 [:state :disposition])))
          (is (not (some #{:prodhab-registration-scope-mismatch} (-> (store/ledger db) last :basis))))
          (is (true? (:submitted? (store/engagement db "eng-5")))))))))

(deftest prodhab-registration-scope-mismatch-does-not-fire-for-exempt-financial-institution
  (testing "SCOPE-CONDITIONAL exemption: an unregistered but exempt regulated financial institution must NOT trigger the flagship check"
    (let [[db actor] (fresh)
          _ (assess! actor "t5cpre" "eng-6")
          _ (draft! actor "t5cpre" "eng-6")]
      (is (= :distribute (:data-handling-scope (store/engagement db "eng-6"))) "fixture declares a triggering scope")
      (is (true? (:regulated-financial-institution? (store/engagement db "eng-6"))) "fixture is an exempt financial institution")
      (is (false? (:prodhab-registered? (store/engagement db "eng-6"))) "fixture is genuinely unregistered")
      (let [r1 (exec-op actor "t5c" {:op :filing/submit :subject "eng-6"} operator)]
        (is (= :interrupted (:status r1)) "clean submit still escalates for ordinary human sign-off, NOT a HARD hold")
        (let [r2 (approve! actor "t5c")]
          (is (= :commit (get-in r2 [:state :disposition])))
          (is (not (some #{:prodhab-registration-scope-mismatch} (-> (store/ledger db) last :basis)))))))))
...
(deftest filing-draft-and-submit-never-auto-commit
  (testing "no phase auto-commits filing/draft or filing/submit -- always human-gated, both structurally (phase.cljc) and via governor high-stakes"
    (let [[db actor] (fresh)
          ...
          r1 (exec-op actor "t14" {:op :filing/draft :subject "eng-7"} operator)]
      (is (= :interrupted (:status r1)) "draft never auto-commits, even governor-clean, even at phase 3")
      (is (false? (:drafted? (store/engagement db "eng-7"))) "not committed until approved")
      (let [_ (approve! actor "t14")
            r2 (exec-op actor "t14b" {:op :filing/submit :subject "eng-7"} operator)]
        (is (= :interrupted (:status r2)) "submit never auto-commits either")
        (is (false? (:submitted? (store/engagement db "eng-7"))) "not committed until approved")))))
