...
(defn- exercise [s]
  (store/commit-record! s {:effect :engagement/upsert
                           :value {:id "eng-x" :operator "X Co" :jurisdiction "CRI"
                                   :base-fee 100 :monthly-rate 10 :monitoring-months 1
                                   :claimed-fee 110.0
                                   :data-handling-scope :commercialize
                                   :regulated-financial-institution? false
                                   :prodhab-registered? true
                                   :drafted? false :submitted? false :status :intake}})
  (store/commit-record! s {:effect :assessment/set
                           :path ["eng-x"]
                           :payload {:jurisdiction "CRI" :checklist ["a"] :spec-basis "x"}})
...
(deftest mem-and-datomic-parity
  (let [mem (store/seed-db)
    ...
    (is (= (:data-handling-scope (:engagement m)) (:data-handling-scope (:engagement d))))
    (is (true? (:prodhab-registered? (:engagement m))) (true? (:prodhab-registered? (:engagement d))))
