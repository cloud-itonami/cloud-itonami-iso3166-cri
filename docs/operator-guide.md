...
2. Register the client's intake: business type, target public function,
   prior filing history in Costa Rica if any.
3. Run the advisor in read-only mode against SICOP (Sistema Integrado de
   Compras Públicas).
...
- named referral relationship with Costa Rican-licensed counsel or a
...
a working referral relationship with Costa Rican-licensed counsel or a
registered agent for whatever licensed representation the law of
Costa Rica requires for actual public-procurement filings.
