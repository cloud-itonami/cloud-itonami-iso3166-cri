# Business Model: Independent Public-Sector Market-Entry & Procurement Compliance Service — Costa Rica

## Classification

- Repository: `cloud-itonami-iso3166-cri`
- ISO 3166: `CRI` (Costa Rica)
...
- Social impact: [:costa-rican-sme-market-access :public-spend-transparency :cross-border-friction-reduction]

## Customer

- an already-incorporated `cloud-itonami-cofog-{code}` /
  `cloud-itonami-isco-{code}` / `cloud-itonami-unspsc-{segment}` /
  `cloud-itonami-{ISIC}` operator wanting to bid on a Costa Rican
  public contract
...
- registration walkthrough for SICOP (Sistema Integrado de Compras
  Públicas), Costa Rica's integrated public-procurement system covering
  central-government ministries, autonomous institutions and
  municipalities, including the largest state-owned enterprises (ICE,
  CCSS, INS)
- business/tax registration checklist: a corporate entry with the
  Registro Nacional de la Propiedad (National Registry, supervised by
  the Ministry of Justice and Peace), required before registering as a
  SICOP supplier
- local-content / preferential-procurement navigation: Costa Rica's
...
- **`cloud-itonami-cofog-{code}`**: a jurisdiction-agnostic operator
  ...
  blueprint AND this one to operate in Costa Rica).
