(ns marketentry.marketentryllm
  ...
  the CRI public-sector market-entry compliance actor.
  ...
  cited), never a committed record or a real SICOP portal submission.
...
(defn- propose-submit
  ...
  :actuation/submit-filing` -- real-world SICOP portal submission."
  [db {:keys [subject]}]
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向けポータル提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "data-handling-scope=" (:data-handling-scope e)
                        " regulated-financial-institution?=" (:regulated-financial-institution? e)
                        " prodhab-registered?=" (:prodhab-registered? e)
     ...
     :confidence (if (and e
                          (or (:prodhab-registered? e)
                              (not (facts/prodhab-registration-required?
                                    (:jurisdiction e) (:data-handling-scope e)
                                    (:regulated-financial-institution? e)))))
                   0.9 0.3)}))
