(ns marketentry.store
  "SSoT for the CRI market-entry compliance actor, behind a `Store`
  ...
  Each engagement also carries `:data-handling-scope`
  (`:internal-only`/`:distribute`/`:disclose`/`:commercialize`),
  `:regulated-financial-institution?` and `:prodhab-registered?` --
  the ground truth `marketentry.governor`'s flagship PRODHAB
  registration-scope check independently verifies. This is
  SCOPE-CONDITIONAL: only a scope that actually triggers the Ley
  N.º 8968 registration duty, on an engagement that is not an exempt
  regulated financial institution, and that has NOT registered, is a
  violation -- never a blanket rule against any particular scope.
...
(defn demo-data
  ...
  eng-4 vs eng-5 vs eng-6 is the flagship contrasting triple: eng-4
  declares a `:commercialize` data-handling scope and is NOT
  registered with PRODHAB -> HARD hold. eng-5 declares the SAME
  `:commercialize` scope but IS registered -> clean, proving the
  check does not false-positive on a legitimate registered case.
  eng-6 declares a `:distribute` scope but is an exempt
  `:regulated-financial-institution?` and is NOT registered -> also
  clean, proving the Ley 8968 financial-institution exemption itself
  suppresses the check rather than a blanket unregistered-database
  rule. eng-1/eng-7 declare `:internal-only` scope (never triggers
  the duty at all)."
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Consultores San José S.R.L." :portal "SICOP"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :data-handling-scope :internal-only :regulated-financial-institution? false :prodhab-registered? false
             :drafted? false :submitted? false
             :jurisdiction "CRI" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "SICOP"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :data-handling-scope :internal-only :regulated-financial-institution? false :prodhab-registered? false
             :drafted? false :submitted? false
             :jurisdiction "ATL" :status :intake}
    "eng-3" {:id "eng-3" :operator "Servicios Heredia Ltda." :portal "SICOP"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 999000.0
             :data-handling-scope :internal-only :regulated-financial-institution? false :prodhab-registered? false
             :drafted? false :submitted? false
             :jurisdiction "CRI" :status :intake}
    "eng-4" {:id "eng-4" :operator "Datos Alajuela Analytics S.A." :portal "SICOP"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :data-handling-scope :commercialize :regulated-financial-institution? false :prodhab-registered? false
             :drafted? false :submitted? false
             :jurisdiction "CRI" :status :intake}
    "eng-5" {:id "eng-5" :operator "Cartago Marketing Data S.A." :portal "SICOP"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :data-handling-scope :commercialize :regulated-financial-institution? false :prodhab-registered? true
             :drafted? false :submitted? false
             :jurisdiction "CRI" :status :intake}
    "eng-6" {:id "eng-6" :operator "Banco Puntarenas Financiero S.A." :portal "SICOP"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :data-handling-scope :distribute :regulated-financial-institution? true :prodhab-registered? false
             :drafted? false :submitted? false
             :jurisdiction "CRI" :status :intake}
    "eng-7" {:id "eng-7" :operator "Obras Públicas de Limón" :portal "SICOP"
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
                               data-handling-scope regulated-financial-institution? prodhab-registered?
                               drafted? submitted?
  ...
  (cond-> {:engagement/id id}
    ...
    data-handling-scope                   (assoc :engagement/data-handling-scope data-handling-scope)
    (some? regulated-financial-institution?) (assoc :engagement/regulated-financial-institution? regulated-financial-institution?)
    (some? prodhab-registered?)           (assoc :engagement/prodhab-registered? prodhab-registered?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/data-handling-scope :engagement/regulated-financial-institution? :engagement/prodhab-registered?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :data-handling-scope (:engagement/data-handling-scope m)
     :regulated-financial-institution? (boolean (:engagement/regulated-financial-institution? m))
     :prodhab-registered? (boolean (:engagement/prodhab-registered? m))
...
(defn kotobase-store
  ...
  [{:keys [db-api seq-fn]}]
  (->KotobaseStore (persist/store {:db-api db-api :actor "marketentry-cri"})
