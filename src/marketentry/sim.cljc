(ns marketentry.sim
  ...
  commit), then shows HARD-hold scenarios, including the flagship
  PRODHAB registration-scope mismatch and its scope-conditional
  non-trigger for internal-only use and for an exempt regulated
...
(defn -main [& _]
  (let [db (store/seed-db)
        actor (op/build db)]
    (println "== engagement/intake eng-1 (CRI, clean, internal-only) ==")
    ...
    (println "== jurisdiction/assess eng-4 (commercialize scope, sets up PRODHAB mismatch) ==")
    ...
    (println "== filing/submit eng-4 (commercialize scope, not PRODHAB-registered -> HARD hold, flagship) ==")
    (println (exec-op actor "t9" {:op :filing/submit :subject "eng-4"} operator))

    (println "== jurisdiction/assess eng-5 (commercialize scope, PRODHAB-registered -- must NOT fire) ==")
    ...
    (println "== filing/submit eng-5 (clean -- escalates for ordinary human sign-off, not a HARD hold) ==")
    (let [r (exec-op actor "t11" {:op :filing/submit :subject "eng-5"} operator)]
      (println r)
      (println (approve! actor "t11")))

    (println "== jurisdiction/assess eng-6 (distribute scope, exempt regulated financial institution -- must NOT fire) ==")
    ...
    (println "== filing/submit eng-6 (unregistered but exempt financial institution -- clean) ==")
